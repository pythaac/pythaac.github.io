...
현금영수증 문제 또한 커피 문제와 같이 해결이 가능하다. 그런데, _**_현금영수증과 같은 간단한 문제에 대해서도 Sum 타입으로 해결해야할까? 이는 안전성과 비용을 따져봐야한다.**
- 128가지의 경우
...
### 안전성이란
여기서 이야기하는 안전성은 무엇일까? 영상에서 **포카요케**를 이야기한다. 포카요케란 실수를 피하다는 뜻의 일본어이며, 도요타에서 사용했다고 한다.
...
포카요케는 도메인의 항상성(Domain invariants), 즉 도메인에서 항상 유지되어야 하는 규칙 또는 조건을 말하며, 이는 도메인의 안전성으로 표현할 수 있다. 안전한 도메인 모델을 위해 타입시스템을 활용할 수 있다는 것이다.
